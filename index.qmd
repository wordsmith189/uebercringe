...
**handout: <br>https://bit.ly/cringe-lh-handout**

# Introduction {background-image="img/river.png"}

## {.smaller}
### CRINGE as a phatic device

:::{.incremental}
- The humor inherent in CRINGE talk (CT) makes speakers volunteer these topics precisely for phaticity's sake (Jefferson 1984).
...
<br>&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;("place talk"; Schegloff 1972, Zhao & Lou 2025) <br>&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;(open-ended)
...
# Findings {background-image="img/road.png"}
...
:::{.fragment}
CRINGE
  : A pop-culturally grounded concept. <br>Only for the young?</br></br>
:::
...
1. Factors like familiarity, general openness, and interviewer persona seemed more important.
...
```{r}
#| fig.cap = "Political Leaning by Rurality"

my_custom_colors2 <- c("#F08080", "#ADD8E6")

df |> 
  filter(political_leaning %in% c("conservative", "progressive")) |> 
  count(rural_urban, political_leaning) |> 
  group_by(rural_urban) |>
  mutate(pct = n / sum(n)) |>
  ungroup() |> 
  
  ggplot(aes(x = rural_urban, y = n, 
  ...
  scale_y_continuous(labels = percent) +
  scale_fill_manual(values = my_custom_colors2) +
  labs(y = NULL, x = NULL, caption = bquote(italic('N') ~ "= 31")) +
  #coord_flip() +
